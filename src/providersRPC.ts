...
const createProviderGetter = <P extends typeof JsonRpcProvider>(Provider: P) => {
  const cache = new Map<string, InstanceType<P>>()

  return (chainId: CHAINS, url: string, cacheSeed = 0, pollingInterval: number | null = null): InstanceType<P> => {
